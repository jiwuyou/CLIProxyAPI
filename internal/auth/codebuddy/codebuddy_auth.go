...
func (a *CodeBuddyAuth) FetchAuthState(ctx context.Context) (*AuthState, error) {
	...
	return &AuthState{
		...
		AuthURL: normalizeCodeBuddyLoginURL(result.Data.AuthURL, result.Data.State),
	}, nil
}

func normalizeCodeBuddyLoginURL(rawURL, state string) string {
	parsed, err := url.Parse(strings.TrimSpace(rawURL))
	if err != nil || !strings.EqualFold(parsed.Hostname(), "copilot.tencent.com") {
		return rawURL
	}
	loginURL := &url.URL{Scheme: "https", Host: DefaultDomain, Path: "/login/"}
