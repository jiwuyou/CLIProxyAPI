...
func TestNormalizeCodeBuddyLoginURL(t *testing.T) {
	got := normalizeCodeBuddyLoginURL("https://copilot.tencent.com/login?state=old", "state-123")
	want := "https://www.codebuddy.cn/login/?platform=CLI&state=state-123"
	if got != want {
		t.Fatalf("normalizeCodeBuddyLoginURL() = %q, want %q", got, want)
	}
}
